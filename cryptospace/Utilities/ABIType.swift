...
extension FixedLengthBytes: ABIType {
    
    public static func decode(bytes: BytesScalar) throws -> FixedLengthBytes {
        return FixedLengthBytes(origin: bytes, length: 32)
    }
    
    public static func decode(message: ABIMessage, index: Int) throws -> FixedLengthBytes {
        return FixedLengthBytes(
            origin: DecodedABIFixedBytes(
                abiMessage: message,
                length: 32,
                index: index
            ),
...
extension SimpleBytes: ABIType {
    
    public static func decode(bytes: BytesScalar) throws -> SimpleBytes {
        return SimpleBytes(bytes: try bytes.value())
    }
    
    public static func decode(message: ABIMessage, index: Int) throws -> SimpleBytes {
        return SimpleBytes(
            bytes: try DecodedABIVariableBytes(
                abiMessage: message,
                index: index
            ).value()
        )
